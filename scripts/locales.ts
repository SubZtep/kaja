...
function valueEnd(text: string, start: number): number {
  for (const fence of ['"""', "'''"]) {
    if (!text.startsWith(fence, start)) continue
    let i = start + 3
    for (;;) {
...
async function syncDir(dir: string, write = true): Promise<string[]> {
  const files = (await readdir(dir)).filter(f => /\.(toml|json)$/.test(f))
  ...
  for (const file of files) {
    if (file === source || !file.endsWith(json ? ".json" : ".toml")) continue
    const locale = file.replace(/\.(toml|json)$/, "")
    ...
    if (json) {
      ...
      for (const [key, value] of english) {
        out[key] = key.startsWith("$") ? value : keep(key) ? (current.get(key) as string) : lorem(value, locale)
      }
...
const paramsOf = (value: string) => [...(value.match(/\{[^{}]+\}/g) ?? [])].sort().join(" ")
...
async function todo(): Promise<Todo[]> {
  ...
  for (const dir of LOCALE_DIRS) {
    const files = (await readdir(dir)).filter(f => /\.(toml|json)$/.test(f)).sort()
    const source = files.find(f => f.startsWith(`${SOURCE}.`)) as string
    const json = source.endsWith(".json")
    const english = valuesOf(await Bun.file(join(dir, source)).text(), json)
    for (const file of files) {
      if (file === source) continue
      const locale = file.replace(/\.(toml|json)$/, "")
      const values = valuesOf(await Bun.file(join(dir, file)).text(), json)
      for (const [key, value] of values) {
        if (!isPlaceholder(value, locale)) continue
        const entry = { file: join(dir, file), key, english: english.get(key) ?? "" }
        todos.push({ ...entry, nearby: nearby(key, english, values, locale) })
      }
    }
  }
  return todos
...
async function apply(input: string): Promise<void> {
  ...
  const byFile = Map.groupBy(translations, t => t.file)
  const errors: string[] = []
  for (const [file, entries] of byFile) {
    ...
    const english = valuesOf(await Bun.file(join(dir, `${SOURCE}.${json ? "json" : "toml"}`)).text(), json)
    const ok = entries.filter(({ key, value }) => {
    ...
    if (json) {
      ...
      for (const { key, value } of ok) out[key] = value
      text = `${JSON.stringify(out, null, 2)}\n`
    } else {
      const values = new Map(ok.map(t => [t.key, t.value]))
      for (const span of scanToml(text).reverse()) {
        const value = values.get(span.key)
        if (value !== undefined) text = text.slice(0, span.start) + tomlString(value) + text.slice(span.end)
      }
    }
...
async function check(): Promise<void> {
  ...
  if (untranslated.size > 0 || drifted.length > 0) {
    console.error(
