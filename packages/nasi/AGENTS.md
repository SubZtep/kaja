...
- The system prompt is built when a conversation starts; after that, `refreshSkillsInPrompt` (every turn, in `run`) rebuilds it only when the `## Skills` section no longer matches the enabled skills, so toggles reach running conversations without breaking prompt caching.
