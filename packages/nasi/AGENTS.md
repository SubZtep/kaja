...
src/
  ...
  packages/          # PackageStore interface, folder store, loadPackages, load_skill (skills), HTTP tool executor, MCP package targets
...
- MCP servers (mcp.toml's and packages', via `createTools`' `mcpPackages`) connect in parallel, each with a timeout (`mcpConnectTimeoutMs`, default 10 s). `connectMcpServer` takes `transport` (http/sse), an `allow` list and `approval` (never/writes/always; `writes` uses the tool's `readOnlyHint`).
