...
test("remember_note creates a new note", async () => {
  ...
  const list = await listNotesTool.execute({}, ctx())
...
test("recall_memory returns a no-match message", async () => {
  const result = await recallMemoryTool.execute({ query: "nonexistent" }, ctx())
...
test("forget_note removes the right key", async () => {
  ...
  const result = await forgetNoteTool.execute({ key: "drop" }, ctx())
...
test("forget_note on a missing key returns a message instead of throwing", async () => {
  const result = await forgetNoteTool.execute({ key: "nope" }, ctx())
...
test("list_notes on an empty store", async () => {
  const result = await listNotesTool.execute({}, ctx())
...
test("recall_memory results carry importance, sticky, tags, and date metadata", async () => {
  ...
  const result = await recallMemoryTool.execute({ query: "brevity" }, ctx())
...
test("list_notes hides content by default and shows it with full: true", async () => {
  ...
  const compact = await listNotesTool.execute({}, ctx())
  expect(compact).toContain("a [low] (tags: x)")
  expect(compact).not.toContain("the secret content")

  const full = await listNotesTool.execute({ full: true }, ctx())
...
test("forget_note deletes in bulk by tag", async () => {
  ...
  const result = await forgetNoteTool.execute({ tag: "test" }, ctx())
...
test("forget_note deletes in bulk by key glob pattern", async () => {
  ...
  const result = await forgetNoteTool.execute({ pattern: "test:*" }, ctx())
...
test("recall_memory with an empty query is uncapped unless limit is passed", async () => {
  ...
  const all = (await recallMemoryTool.execute({ query: "" }, ctx())) as string
  expect(all.split("\n")).toHaveLength(7)
...
test("forget_note requires exactly one selector", async () => {
  expect(await forgetNoteTool.execute({}, ctx())).toBe("Provide exactly one of: key, tag, pattern.")
  expect(await forgetNoteTool.execute({ key: "a", tag: "b" }, ctx())).toBe("Provide exactly one of: key, tag, pattern.")
})
