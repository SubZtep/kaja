...
import { createLoadSkillTool, skillsForPersona } from "../../src/abilities/skills"
import { type AbilityStore, SkillFileError, type SkillSummary } from "../../src/abilities/types"
...
function fakeStore(skills: SkillSummary[], files: Record<string, string> = {}): AbilityStore {
  return {
...
test("loadAbilities adds no tool when no skill is enabled", async () => {
  expect(await loadAbilities(fakeStore([]))).toEqual({
    groups: [],
    skills: [],
    httpTools: [],
    mcp: [],
...
test("loadAbilities survives a store that throws", async () => {
  const broken: AbilityStore = {
    listSkills: async () => {
...
test("loadAbilities adds load_skill as an official tool carrying the skill list", async () => {
  const { groups, skills } = await loadAbilities(fakeStore([pdf]))
