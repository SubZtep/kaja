...
describe("personas in the cloud", () => {
  ...
  const toggle = (method: "PUT" | "DELETE", name: string) =>
    app.request(`/packages/me/persona/${name}`, { method, headers: auth() })
  const roster = async () =>
    ((await (await app.request("/nasi/info", { headers: auth() })).json()).personas as { id: string }[]).map(p => p.id)

  beforeAll(async () => {
    base = mkdtempSync(join(tmpdir(), "kaja-package-personas-test-"))
  ...
  afterAll(async () => {
    ...
    await pool.query("DELETE FROM package WHERE type = 'persona' AND name = 'default' AND description = $1", [
      FIXTURE_DEFAULT
    ])
  ...
  test("the catalog shows who each persona is, and leaves out default, which is always on", async () => {
    ...
    const personas = packages.filter((pkg: { type: string }) => pkg.type === "persona")
    expect(personas.find((pkg: { name: string }) => pkg.name === care)).toMatchObject({
      description: "Care",
      persona: { label: "Care", when: "the user is sad", instructions: "Be kind." }
    })
    expect(personas.map((pkg: { name: string }) => pkg.name)).not.toContain("default")
  })
  ...
  test("a persona that left the marketplace shows as unavailable and leaves the roster", async () => {
    ...
    const { packages } = await (await app.request("/packages/me", { headers: auth() })).json()
    expect(packages.find((pkg: { name: string }) => pkg.name === care)).toMatchObject({ available: false })
    expect(await roster()).toEqual(["default"])
  })
})
