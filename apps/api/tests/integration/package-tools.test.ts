...
async function savedCall(callId: string) {
  const { rows } = await pool.query(
    `SELECT tc.approval, r.content AS result
...
describe("HTTP tools in the cloud", () => {
  ...
  const saveKey = (name: string, apiKey: string, as = token) =>
    app.request(`/packages/me/tool/${name}/key`, { method: "PUT", headers: auth(as), body: JSON.stringify({ apiKey }) })
  const enable = (type: string, name: string, as = token) =>
    app.request(`/packages/me/${type}/${name}`, { method: "PUT", headers: auth(as) })
  const mine = async (as = token) => (await app.request("/packages/me", { headers: auth(as) })).json()
  ...
  beforeAll(async () => {
    base = mkdtempSync(join(tmpdir(), "kaja-package-tools-test-"))
  ...
  test("the catalog shows where each tool calls, whether it needs a key, and its methods", async () => {
    ...
    const byName = (name: string) => packages.find((pkg: { name: string }) => pkg.name === name)
  ...
  test("a cloud turn gets the user's tools; a POST waits for approval, and approving runs the call the server saved", async () => {
    ...
    expect(await savedCall("call_a")).toEqual({ approval: "approved", result: 'HTTP 201\n\n{"id":7}' })
    ...
    const { providerId } = await seedModel("package-tools-info")
    try {
  ...
  test("the stream forwards confirm_tool, and declining never calls the tool", async () => {
    ...
    expect(requests).toEqual([])
    expect(await savedCall("call_b")).toEqual({ approval: "declined", result: "User declined this request." })
  })
  ...
  test("keys are per user: another user sees none, and a copied row doesn't decrypt for them", async () => {
    ...
    await pool.query(
      ...
      [otherId, userId, `package:${issues}`]
    )
    expect(await secretService.get(otherId, `package:${issues}`)).toBeUndefined()
    expect(await secretService.get(userId, `package:${issues}`)).toBe(GOOD_KEY)
  })

  test("removing the key turns off a tool that can't work without it", async () => {
    const removed = await app.request(`/packages/me/tool/${issues}/key`, { method: "DELETE", headers: auth() })
    expect(removed.status).toBe(200)
    const listed = await mine()
    expect(listed.keys).not.toContain(issues)
    expect(listed.packages.map((pkg: { name: string }) => pkg.name)).not.toContain(issues)
    expect(listed.packages.map((pkg: { name: string }) => pkg.name)).toContain(weather)
  })
  ...
  test("without USER_SECRET_KEY, key entry is off and tools that need a key are hidden", async () => {
    ...
    try {
      ...
      const names = ((await (await app.request("/packages")).json()).packages as { name: string }[]).map(p => p.name)
