...
/** A marketplace folder with one usable MCP package and four the cloud must skip. */
...
describe("MCP servers in the cloud", () => {
  ...
  const saveKey = (apiKey: string) =>
    app.request(`/packages/me/mcp/${things}/key`, { method: "PUT", headers: auth(), body: JSON.stringify({ apiKey }) })
  const turn = (body: object) =>
    app.request("/nasi/turn", { method: "POST", headers: auth(), body: JSON.stringify(body) })

  beforeAll(async () => {
    base = mkdtempSync(join(tmpdir(), "kaja-package-mcp-test-"))
  ...
  test("a sync adds remote MCP packages with a tool list; stdio, private, unlisted and clashing ones are skipped", async () => {
  ...
  test("the catalog shows where an MCP server runs, its key need, when it asks, and its tools", async () => {
    ...
    expect(packages.find((pkg: { name: string }) => pkg.name === things)).toMatchObject({
      type: "mcp",
      mcp: {
        domain: host,
        key: "required",
  ...
  test("it needs a key before it can be turned on; a saved key is tested by connecting", async () => {
    const refused = await app.request(`/packages/me/mcp/${things}`, { method: "PUT", headers: auth() })
    ...
    expect((await app.request(`/packages/me/mcp/${things}`, { method: "PUT", headers: auth() })).status).toBe(200)
  })
  ...
  test("a cloud turn connects the server with the user's key; a write waits for approval, then runs", async () => {
    ...
    expect(paused.steps).toContainEqual({
      ...
      name: "write_thing",
      arguments: '{"id":"2"}',
      summary: `package:${things} write_thing {"id":"2"}`
    })
    ...
    const { providerId } = await seedModel("package-mcp-info")
    try {
  ...
  test("removing the key turns off a server that can't work without it", async () => {
    const removed = await app.request(`/packages/me/mcp/${things}/key`, { method: "DELETE", headers: auth() })
    expect(removed.status).toBe(200)
    const mine = await (await app.request("/packages/me", { headers: auth() })).json()
    expect(mine.keys).not.toContain(things)
    expect(mine.packages.map((pkg: { name: string }) => pkg.name)).not.toContain(things)
  })
})
