...
import { marketplaceService, packageService, secretService } from "../../src/services"
...
describe("the cloud bot's /packages", () => {
  let base: string
  let userId: string
  ...
  const list = () => sent.findLast(message => message.text.startsWith("<b>Packages</b>"))!
  const tap = (name: string, as = LINKED) => driver.handleCallback(as, 1, list().id, button(name)!.data)
  const enabled = async () => (await packageService.listForUser(userId)).map(pkg => pkg.name)

  beforeAll(async () => {
    base = mkdtempSync(join(tmpdir(), "kaja-telegram-packages-test-"))
  ...
  afterAll(async () => {
    ...
    // The sync above marked the real marketplace's packages removed; forget the stored commit so the next real sync re-applies it.
  ...
  test("lists the catalog as buttons, skills then personas then tools, with a key marker, a web link and pages", async () => {
    ...
    await driver.handleCallback(LINKED, 1, first.id, "pkgp:1")
  ...
  test("a tap turns a package on or off for the user who tapped, and redraws the list", async () => {
    await tap(keyless)
  ...
  test("a package that needs a key points to the web instead of asking for it here", async () => {
    await tap(keyed)
    expect(sent.at(-1)!.text).toContain(`${keyed}</b> needs your API key`)
    expect(sent.at(-1)!.text).toContain("/packages?tab=tools")
    expect(await enabled()).not.toContain(keyed)

    await secretService.set(userId, `package:${keyed}`, "a-key")
    await driver.handleMessage(LINKED, 1, "/packages")
    await driver.handleCallback(LINKED, 1, list().id, "pkgp:1")
    expect(button(keyed)!.text).toBe(`▫️ ${keyed} · tool`)
    await tap(keyed)
    expect(await enabled()).toContain(keyed)
  })

  test("a package that left the marketplace shows as gone, and a tap turns it off", async () => {
    await tap(keyless)
