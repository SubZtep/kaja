...
Users don't save ability keys in the cloud: `ABILITY_KEYS` (temporary, `name=key,...`, TODO: admin-managed service keys) gives an ability a server-wide key every user shares (`AbilityService.serviceKey`); a tool or MCP server that requires a key and has none there is hidden (`#usable`). A tool that isn't GET pauses the turn (`confirm_tool` step, `needs_approval`); the next turn's `approval: "approve" | "decline"` makes Nasi run or skip the call the session saved. The cloud Telegram bot answers it with inline buttons (`tool:approve|decline:<hash of the call id>`), matched against the pressing user's latest session. Its `/abilities` (`features/telegram/abilities.ts`) lists the catalog as buttons (`ability:<s|t|m>:<name hash>:<page>`, `abilitypage:<page>`) that toggle the pressing user's own abilities; Both bots register a `/new` + `/abilities` command menu on start.
