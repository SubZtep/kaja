...
/**
 * {@link PackageStore} over the `package` table for one cloud turn. Skills only (HTTP tools and MCP
 * aren't in the cloud yet); files come from the stored bundle, so there's no disk and no skill folder
 * path, and skills with scripts never get here (the catalog doesn't offer them).
 */
