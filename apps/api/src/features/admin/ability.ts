...
const syncNowRoute = createRoute({
  ...
  path: "/abilities/sync",
  tags: ["Admin"],
  summary: "Sync the ability catalog from the marketplace repo now (skipped when the branch hasn't moved)",
  security: [{ bearerAuth: [] }],
...
export function registerAdminAbilities(app: RouteRegProps) {
