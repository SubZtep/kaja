import { bool, parseEnv, positiveInt, trimmed, url } from "@kaja/schema/env"
...
export const EnvSchema = z.object({
  ...
  BETTER_AUTH_SECRET: trimmed.describe("Better Auth signing secret").meta({ secret: true }),
  ...
  CONFIG_API_TOKEN: trimmed
    ...
    .meta({ secret: true, example: "kaja" }),

  BUN_TEST: trimmed.optional().describe("Set by the Bun test runner; disables rate limiting when present"),
  RATE_LIMIT_ENABLED: bool.optional().describe("Set false to disable global + auth + nasi/widget turn rate limiters"),
  RATE_LIMIT_WINDOW_MS: positiveInt.default(15 * 60 * 1000).describe("Global rate limit window (ms)"),
  RATE_LIMIT_MAX: positiveInt.default(1000).describe("Max requests per window per IP"),
  AUTH_RATE_LIMIT_WINDOW_MS: positiveInt.default(15 * 60 * 1000).describe("Auth rate limit window (ms)"),
  AUTH_RATE_LIMIT_MAX: positiveInt.default(2000).describe("Max auth requests per window per IP"),
  NASI_TURN_RATE_LIMIT_WINDOW_MS: positiveInt
    .default(10 * 60 * 1000)
    .describe("/nasi/turn(/stream) rate limit window (ms)"),
  NASI_TURN_RATE_LIMIT_MAX: positiveInt.default(1000).describe("Max turns per window per user"),
  WIDGET_TURN_RATE_LIMIT_WINDOW_MS: positiveInt.default(10 * 60 * 1000).describe("/widget/turn rate limit window (ms)"),
  WIDGET_TURN_RATE_LIMIT_MAX: positiveInt.default(300).describe("Max turns per window per (widget key, IP) pair"),
