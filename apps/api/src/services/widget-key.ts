...
export class WidgetKeyService {
  ...
  async createKey(userId: string, label: string, allowedOrigins: string[]): Promise<WidgetKey & { rawKey: string }> {
    const rawKey = `${KEY_PREFIX}${randomBytes(24).toString("base64url")}`
    const result = await this.#db.query(
      `
      INSERT INTO widget_key (user_id, label, key_prefix, key_hash, allowed_origins)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
      `,
      [userId, label, rawKey.slice(0, 12), hashKey(rawKey), allowedOrigins]
    )
  ...
  async resolveByRawKey(rawKey: string): Promise<{ id: string; userId: string; allowedOrigins: string[] } | null> {
    const { rows } = await this.#db.query(
      `SELECT id, user_id, allowed_origins FROM widget_key WHERE key_hash = $1 AND enabled`,
      [hashKey(rawKey)]
    )
    const row = rows[0]
    if (!row) return null
    return { id: row.id, userId: row.user_id, allowedOrigins: row.allowed_origins }
  }
