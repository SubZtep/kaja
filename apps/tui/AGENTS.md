...
- First run (no `settings.toml` yet, interactive TTY only): `components/config-wizard.tsx` — the same wizard `kaja config wizard` re-runs, so there is one setup flow, not two. Steps are language → mode → provider → server address → abilities → extras → summary; a step is skipped when it can't apply (cloud needs no provider, and only a provider that runs on this machine — Ollama, llama.cpp — is asked for an address), and `--cloud`/`--local` skip the mode step. Language is first and never skipped, because every question after it is only answerable by someone who can read it; picking one calls `setLanguage` straight away, so the rest of the wizard renders in it (the caller still writes `preferences.locale`). A *prefilled* mode does not skip the mode step — that's the current setting being re-offered — only a forced one does, which is why `nextStepAfter` takes the forced mode rather than reading `result.mode`. **No step asks for an API key**: once `applyResult` has written the files and `applyAbilities` has written abilities.toml, `runCredentialPass` (`lib/doctor/credentials.ts`, shared with `kaja doctor`) asks for and live-tests every key the finished config needs — provider keys and the newly enabled abilities' keys included — so a key is never saved untested. The abilities step only records a choice (`"starter" | "pick" | "none"`); the marketplace sync, scan and checklist all happen in `applyAbilities` afterwards, since the component itself does no I/O. `"starter"` is `starterSelection` in `lib/abilities/picker.tsx` — every ability needing no key or only an optional one, **excluding stdio MCP servers** (enabling one spawns a process, which stays behind `confirmStdioServers` in `kaja abilities`) — and it is merged into what is already enabled rather than replacing it. `scanMarketplace`/`pickAbilities`/`confirmStdioServers`/`ensureMarketplace` live in that same module, shared with `subcommands/abilities.tsx`. The extras step (web search, voice, Telegram — a MultiSelect with nothing ticked, so one Enter skips it) is applied by `applyExtras`, which writes only non-secret config and hands the credential pass a list of items `collectCredentials` can't discover: web search has no services.toml section to look for, and Telegram's is only written once there's a real user id, because `allowedUserIds` is `min(1)` and an invalid services.toml makes `loadServicesFile` exit on every later run. After the extras, `offerModelDownloads` asks once whether to download the models `models.toml` names that the server hasn't got (`lib/models/pull.ts`: `/api/tags` to see what's installed, `/api/pull` streaming ndjson to fetch each). It is one question for the whole list, never one per model, because nothing about them is a choice — the config already names them. A server that 404s on `/api/tags` (llama.cpp, any cloud provider, one that isn't running) is skipped silently, so the question only appears when it can be acted on; it runs before the credential pass so the provider probe meets a model that is really there. Voice and Telegram config is added with `appendTomlSection` (`lib/config/toml.ts`), which appends a table instead of re-serializing, so the templates' commented-out examples survive; it never touches a table that already exists. Every step opens on the current value (`readPrefill` in `lib/cli/config-wizard.tsx` reads settings.toml, models.toml's `[models.chat].provider` and secrets.toml), so Enter throughout keeps a configured machine unchanged. Optionally copies `models.fireworks.toml`/`models.ollama.toml` as a starting `models.toml`; "Skip" writes neither, and `subcommands/run.tsx` then exits with an error instead of silently falling back to cloud free chat (`isFreeChat` in `lib/models/openai.ts` — still used internally by `chatModel` resolution, just no longer a reachable default). Non-interactive stdin or `--headless` writes the template untouched, no prompts. Dispatch glue lives in `lib/cli/` (`args.ts`, `bootstrap.ts`, `config-wizard.tsx`).
