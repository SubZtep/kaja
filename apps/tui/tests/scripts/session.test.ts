...
test("list, prefix match, and dump cover the terminal session and leave secrets out", async () => {
  ...
  expect(markdown).toContain("%%{init: {'htmlLabels': false}}%%")
  expect(markdown).toContain('flowchart TD\n    subgraph context["Context"]')
  expect(markdown).toContain('subgraph model["Model"]')
  expect(markdown).toContain('subgraph tools["Tools"]')
  expect(markdown).toContain("classDef tools fill:#dcfce7")
  expect(markdown).toContain('href "#step-system"')
  expect(markdown).toContain('e1["1 user: hi"]')
  expect(markdown).toContain('href "#timeline-1"')
  expect(markdown).toContain('href "#timeline-6"')
