...
// Built in, so a fresh install has a persona before any `kaja pkg update`; the same file the marketplace syncs.
 ...
 * one. Then the personas packages.toml enables, by id; broken or missing files are skipped with a warning. A
...
export async function loadPersonas(): Promise<Persona[]> {
  ...
  const { personas: enabled } = await loadPackagesFile()
