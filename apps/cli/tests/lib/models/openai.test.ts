...
test("createOpenAIClient merges custom headers into outgoing requests without dropping the SDK's own headers", async () => {
  ...
  globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
    // The SDK passes a real Headers instance here, not a plain object — the fake fetch below mirrors that so a spread-based merge bug (which silently drops entries from a Headers instance) is caught.
