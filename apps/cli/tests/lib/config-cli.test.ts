...
afterEach(async () => {
  ...
  await $`rm -f ${getMcpPath()} ${getMcpPath()}.bak ${getMcpPath()}.bak2`.quiet().nothrow()
})
...
test("fetch writes mcp.toml on success", async () => {
  stubFetch('[[servers]]\nid = "x"\n', 200)
