...
function PersonaCard({
  ...
  pending,
  onToggle
}: Readonly<{ persona: CatalogPackage; enabled: boolean; pending: boolean; onToggle: (on: boolean) => void }>) {
...
export function PersonaCards() {
  ...
  const personas = (catalog.data ?? []).filter(pkg => pkg.type === "persona")
  const enabled = new Set((mine.data?.packages ?? []).filter(p => p.type === "persona").map(p => p.name))
  ...
  return (
    <>
      ...
      <UnavailablePackages types={["persona"]} />
    </>
  )
}
